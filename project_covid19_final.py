...
sql = f'''drop view if exists cnt_deaths_province_russia'''
# result = engine.execute(sql)

with engine.begin() as conn:
    conn.execute(text('drop view if exists cnt_deaths_province_russia'))

with engine.begin() as conn:
    conn.execute(text('drop view if exists cnt_confirmed'))
    
with engine.begin() as conn:
    conn.execute(text('drop view if exists cnt_deaths'))
    
with engine.begin() as conn:
    conn.execute(text('drop view if exists cases_deaths'))
    
with engine.begin() as conn:
    conn.execute(text('drop view if exists covid19_cases_deaths'))   
    
with engine.begin() as conn:
    conn.execute(text('drop view if exists covid19_realtime'))
...
with engine.begin() as conn:
    conn.execute(text('''create view cnt_deaths_province_russia as
select "Province_State", "Deaths"::int
from daily_reports
where "Country_Region" = 'Russia'
order by "Deaths"::int desc limit 7'''))
    
# создаем вью (количество зараженных по странам, топ 5)
with engine.begin() as conn:
    conn.execute(text('''create view cnt_confirmed as
...
with engine.begin() as conn:
    conn.execute(text('''create view cnt_deaths as
